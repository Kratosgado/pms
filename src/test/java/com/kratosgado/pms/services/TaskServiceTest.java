...
public class TaskServiceTest {
  ...
  @Test
  void testParallelTaskSimulation() {
    ...
    try {
      ...
      assert (project.findTaskById("TS001").get().getStatus() == TaskStatus.PENDING);
      assert (project.findTaskById("TS002").get().getStatus() == TaskStatus.IN_PROGRESS);
      assert (project.findTaskById("TS003").get().getStatus() == TaskStatus.COMPLETED);
